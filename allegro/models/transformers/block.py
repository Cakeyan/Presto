...
@maybe_allow_in_graph
class PrestoTransformerBlock(nn.Module):
    r"""
    A Presto Transformer block for multiple prompts.
    ...
    def __init__(
        ...
        self.sample_size_t = sample_size_t
    ...
    def forward(
        ...
        if self.attn2 is not None:
            ...
            if self.num_prompts > 1:
                # segmented attention
                sampled_prompt_frame_points = np.linspace(0, self.sample_size_t, self.num_prompts+1, dtype=int, endpoint=True)
                min_max_idx = [(max(0,now-1), min(self.sample_size_t,next+1)) for now, next in zip(sampled_prompt_frame_points[:-1], sampled_prompt_frame_points[1:])]
                norm_hidden_states = rearrange(norm_hidden_states, 'b (p x) c -> b p x c', p=self.sample_size_t)
                nhs_num_smallest = norm_hidden_states.shape[2]
                overlap_window = min_max_idx[0][1] - min_max_idx[1][0]
                attn_output_all = None
                for i, (min_idx, max_idx) in enumerate(min_max_idx):
                    norm_hidden_states_input = norm_hidden_states[:, min_idx:max_idx]
                    norm_hidden_states_input = rearrange(norm_hidden_states_input, 'b l x c -> b (l x) c')
                    attn_output = self.attn2(
                        norm_hidden_states_input,
                        encoder_hidden_states=encoder_hidden_states[:, i],
                        attention_mask=encoder_attention_mask[:, i],
                        **cross_attention_kwargs,
                    )
                    
                    if attn_output_all is None:
                        attn_output_all = attn_output
                    else:
                        attn_output_all[:, -overlap_window*nhs_num_smallest:] += attn_output[:, :overlap_window*nhs_num_smallest]
                        attn_output_all[:, -overlap_window*nhs_num_smallest:] /= 2.
                        attn_output_all = torch.cat([attn_output_all, attn_output[:, overlap_window*nhs_num_smallest:]], dim=1)
                attn_output = attn_output_all
            else:
                attn_output = self.attn2(
                    norm_hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
